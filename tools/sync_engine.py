# -*- coding: utf-8 -*-
"""
引擎同步工具 —— 让 CP1 / CP2 / CP3 共用同一套皮肤引擎, 不再各改一份。
...
def engine_dir(key):
    """引擎文件所在目录: CP1 是包根, CP2/CP3 是包内的 engine/。"""
    v = PACKS[key]
...
def _role_of(key):
    ...
    return {"CP2": "odette_voj", "CP3": "cp3_trio"}.get(key)
...
def normalize(text, src_slug, dst_slug, dst_key=None, src_key=None, src_repo=None,
    ...
    # 注意: 源与目标的角色模块名可能不同(cp3_trio vs odette_voj),
