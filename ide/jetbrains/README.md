# JetBrains 系 IDE (PyCharm / WebStorm / IntelliJ / GoLand) — 原神CP3 背景图
...
1. 生成全部素材:
   ...
   ```bash
   genshen-cp3 all --out "%USERPROFILE%\GenshinCP1-Backgrounds"   # Windows
   genshen-cp3 all --out ~/GenshinCP1-Backgrounds                 # macOS / Linux
   ```

   源码形态: `python -m genshen_skin_cp3.engine.cli all --out 目录`
...
- 想换壁纸时重复第 3 步选另一张即可。
- 与桌面壁纸互不影响: 桌面用 `genshen-cp3 2`, IDE 背景图单独设。
