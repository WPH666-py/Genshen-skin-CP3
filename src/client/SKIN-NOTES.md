...
本目录的 `genshen-cp3.module.css` 同时服务两个消费者，改它之前请读完本文。
...
`genshen-cp3.module.css` 的注释里绝对不能出现：
...
- 想保留夜景蓝紫的逐槽位校色版本，用：
  ...
  ```bash
  genshen-cp3 deepking          # 导出 ~/.genshen-cp3/deepking/*.skin.json
  ```
...
```bash
genshen-cp3 deepking --check    # 应输出「通过」
genshen-cp3 deepking --what     # 看转换器实际会取到什么值
```
...
- `mascot-cp3-light.jpg` —— 取右上那对（米提亚 × 白发少年，浅灰底），配净白界面
- `mascot-cp3-dark.jpg` —— 取左下那对（沃雅妮莎 × 奥黛塔，青蓝发色），配夜景界面

两张都由 `01-trio.jpg` 生成。重做方式：调整 `crop` 的中心比例与取景边长，
