...
def pkg_root():
    """包目录 .../genshen_skin_cp3(本模块在它的 engine/ 下, 故向上一层)。
...
# src/client/genshen-cp3.module.css 完全一致的取值, 保证在线转换与离线生成一致。
        ...
        <section class="mock" style="background:%(bg)s;color:%(bgText)s">
          ...
          <div class="body">
            <aside style="background:%(sidebarBg)s;color:%(sidebarText)s;border-color:%(border)s">
              ...
              <div class="item">📁 Genshen-skin-CP3</div>
...
<h2>🌙 暗色 Dark</h2>%(dark)s
        ...
        "light": region(p, "亮色 · 原神CP3", mascot.get("light")),
        "dark": region(d, "暗色 · 原神CP3", mascot.get("dark")),
    }
