...
try:
    from genshen_skin_cp3.engine import skin_core as sc
...
def make_icon():
    ...
    配色取自群像插画: 米提亚的青绿长发与礼服钴蓝。
    """
    from PIL import Image, ImageDraw
    S = 256
    img = Image.new("RGBA", (S, S), (0, 0, 0, 0))
    d = ImageDraw.Draw(img)
    # 渐变底: 上青绿 -> 下钴蓝(与皮肤 accent #2f9e8f 同源)
    top = (47, 158, 143)      # #2f9e8f
    bottom = (47, 107, 184)   # #2f6bb8
    for y in range(S):
        t = y / (S - 1)
