// 原神 CP 壁纸套件3 — 米提亚 × 奥黛塔 × 沃雅妮莎 · 单张素材三种呈现 (VSCode / Trae / CodeX / Cursor / Windsurf)
...
const REPO_URL = 'https://github.com/WPH666-py/Genshen-skin-CP3.git';
const EXT_ID = 'wp666.genshen-skin-cp3';
...
function detectPython() {
  const cfg = expand(vscode.workspace.getConfiguration('genshencp3').get('pythonPath', ''));
...
function findRepo() {
  const cfg = expand(vscode.workspace.getConfiguration('genshencp3').get('repoPath', ''));
  const cands = [
    cfg,
    process.env.GENSHIN_CP1_REPO,
    path.join(os.homedir(), 'Genshen-skin-CP3'),
    process.env.USERPROFILE ? path.join(process.env.USERPROFILE, 'Genshen-skin-CP3') : null,
  ...
  for (const c of cands) {
    try {
      if (fs.existsSync(path.join(c, 'src', 'genshen_skin_cp3', 'cli.py'))) return c;
...
function buildInvocation(sub) {
  ...
  if (repo) {
    env.PYTHONPATH = (env.PYTHONPATH ? env.PYTHONPATH + path.delimiter : '') +
      path.join(repo, 'src');
    return { py: py, args: ['-m', 'genshen_skin_cp3.cli'].concat(sub), cwd: repo, env: env };
  }
  // pip 安装形态: 直接用模块
  return { py: py, args: ['-m', 'genshen_skin_cp3.cli'].concat(sub), cwd: os.homedir(), env: env };
}

function envError(msg) {
  vscode.window.showWarningMessage(
    msg + ' 请先安装: pip install ' + 'genshen-skin-cp3' +
    '  (或克隆 ' + REPO_URL + ' 到 ' + path.join(os.homedir(), 'Genshen-skin-CP3') + ')'
  );
}
...
function spawnGui(moduleName, label) {
  ...
  const child = spawn(exe, ['-m', 'genshen_skin_cp3.' + moduleName], {
    cwd: inv.cwd,
    env: inv.env,
    detached: true,
    stdio: 'ignore',
...
// 本套件只有一张素材(米提亚 × 米提亚 · 奥黛塔 · 沃雅妮莎 群像), 因此画廊给的是同一张的
// 三种呈现方式。ids 必须与 characters/cp3_trio.py 的 MODES 一致。
const CARDS = [
...
function renderHtml(thumb, hasPython) {
  ...
  return '<!doctype html><html lang="zh-CN"><head><meta charset="utf-8"><style>' +
    ...
    '</div>' +
    '<div class="foot">原神 CP 壁纸套件3 · 米提亚 × 奥黛塔 × 沃雅妮莎 v0.1.0<br/>' +
...
function activate(ctx) {
  ...
  reg('genshencp3.setCard', () => runCli(['card'], '已切换: 卡片式'));
  reg('genshencp3.setCover', () => runCli(['cover'], '已切换: 满屏'));
  reg('genshencp3.setShowall', () => runCli(['showall'], '已切换: 完整不裁'));
  reg('genshencp3.setRandom', () => runCli(['random'], '已随机换一张!'));
  reg('genshencp3.generateAll', () => runCli(['all'], '已生成全部样式!'));
  reg('genshencp3.openSwitcher', () => spawnGui('switcher', '壁纸切换器'));
  reg('genshencp3.startPet', () => spawnGui('pet', '桌面桌宠'));
  reg('genshencp3.showInfo', () => {
    const inv = buildInvocation(['info']);
    if (inv.error) return envError(inv.error);
    const term = vscode.window.createTerminal({ name: '原神CP3 自检', cwd: inv.cwd, env: inv.env });
    term.show();
    term.sendText('"' + inv.py + '" ' + inv.args.map((a) => '"' + a + '"').join(' '));
  });
  reg('genshencp3.openGallery', () =>
    vscode.commands.executeCommand('workbench.view.extension.genshencp3')
  );

  ctx.subscriptions.push(
    vscode.window.registerWebviewViewProvider('genshencp3.gallery', new GalleryProvider(), {
